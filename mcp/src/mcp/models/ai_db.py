...
class QueryResponse(BaseModel):
    ...
    data: Optional[list[dict[str, Any]]] = Field(None, description="Query results for select operations")
    schema: Optional[dict[str, Any]] = Field(None, description="Result schema for data-returning queries")
    data_loss_indicator: DataLossIndicator = Field(..., description="Indicator of potential data loss")
    ai_comment: str = Field(..., description="AI interpretation details")
    compiled_plan: Optional[str] = Field(None, description="Serialized query plan for data-returning queries")
    transaction_id: Optional[str] = Field(None, description="Transaction ID if within transaction")
...
class SchemaResponse(BaseModel):
    ...
    schema: dict[str, Any] = Field(..., description="Current database schema")
