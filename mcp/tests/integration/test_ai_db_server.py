...
from src.ai_db_server import create_ai_db_server
from src.config import AIDBConfig
...
class TestAIDBServer:
    ...
    async def test_list_tools(self, ai_db_config):
        ...
        assert schema_tool.description == "Modify table schemas, manage relationships, and constraints"
    ...
    async def test_transaction_flow(self, ai_db_config):
        ...
        select_result = await call_tool_handler(
            "select",
            {"query": "SELECT * FROM users", "transaction_id": tx_id}
        )
        ...
        commit_result = await call_tool_handler(
            "commit_transaction",
            {"transaction_id": tx_id, "commit_message": "Test commit"}
        )
