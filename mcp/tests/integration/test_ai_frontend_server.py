...
from ai_mcp.ai_frontend_server import create_ai_frontend_server
...
class TestAIFrontendServer:
    ...
    async def test_list_tools(self, ai_frontend_config):
        ...
        assert (
            generate_tool.description
            == "Generate or modify React frontend components using natural language"
        )
    ...
    async def test_call_generate_frontend(self, ai_frontend_config):
        ...
        result = await call_tool_handler(
            "generate_frontend", {"request": "Create a user dashboard with charts"}
        )
