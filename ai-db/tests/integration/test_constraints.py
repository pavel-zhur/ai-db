...
from ai_db.storage import SchemaStore
...
class TestConstraintIntegration:
    ...
    async def test_foreign_key_cascade(self, temp_dir):
        ...
        all_tables_data = {"users": users_data, "orders": valid_orders}

        errors = await checker.check_constraints(
            schema.tables["orders"], valid_orders, all_tables_data
        )

        assert len(errors) == 0

        # Test with invalid foreign key
        invalid_orders = [*valid_orders, {"id": 3, "user_id": 99, "amount": 300}]  # Invalid user_id

        errors = await checker.check_constraints(
            schema.tables["orders"], invalid_orders, {"users": users_data, "orders": invalid_orders}
        )
    ...
    async def test_complex_check_constraint(self, temp_dir):
        ...
        products_table = Table(
            ...
            constraints=[
                Constraint(name="pk_products", type=ConstraintType.PRIMARY_KEY, columns=["id"]),
                Constraint(
                    name="chk_price",
        ...
        errors = await checker.check_constraints(products_table, test_data, {"products": test_data})
    ...
    async def test_multi_column_unique_constraint(self, temp_dir):
        ...
        user_roles_table = Table(
            ...
            constraints=[
                Constraint(name="pk_user_roles", type=ConstraintType.PRIMARY_KEY, columns=["id"]),
                Constraint(
                    name="uk_user_role",
        ...
        errors = await checker.check_constraints(
            user_roles_table, test_data, {"user_roles": test_data}
        )
