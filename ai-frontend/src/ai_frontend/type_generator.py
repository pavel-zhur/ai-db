...
class TypeScriptGenerator:
    ...
    def _generate_table_types(self, table_name: str, table_schema: Dict[str, Any]) -> List[str]:
        ...
        if "columns" in table_schema:
            for col_name, col_schema in table_schema["columns"].items():
                ...
                optional = col_schema.get("nullable", False) or not col_schema.get("required", True)
                field_name = col_name
    ...
    def _generate_view_types(self, view_name: str, view_schema: Dict[str, Any]) -> List[str]:
    ...
    def _generate_dto_types(self, base_type: str, table_schema: Dict[str, Any]) -> List[str]:
        ...
        if "columns" in table_schema:
            for col_name, col_schema in table_schema["columns"].items():
                ...
                ts_type = self._json_schema_to_ts_type(col_schema)
                optional = (
                    col_schema.get("nullable", False) or col_schema.get("default") is not None
                )
        ...
        if "columns" in table_schema:
            for col_name, col_schema in table_schema["columns"].items():
                # Skip immutable fields, auto-increment fields, and primary keys
                if (
                    col_schema.get("immutable")
                    or col_schema.get("auto_increment")
                    or col_schema.get("primary_key")
                ):
                    continue
