...
async def example_session() -> None:
    ...
    from console.config import ConsoleConfig, GitLayerConfig

    # Create configuration
    config = Config(
        git_layer=GitLayerConfig(repo_path="./example_data"),
        console=ConsoleConfig(
            log_file="example.log", trace_file="example_trace.log", debug_mode=True
        ),
    )
    ...
    await interface._execute_db_query(
        CommandType.SCHEMA_MODIFY, "Create a products table with id, name, price, and category"
    )
    print("Table created")

    # Insert some data
    await interface._execute_db_query(
        CommandType.DATA_MODIFY, "Add product 'Laptop' with price 999.99 in category 'Electronics'"
    )
    print("Data inserted")

    # Query the data
    await interface._execute_db_query(CommandType.QUERY, "Show all products")
