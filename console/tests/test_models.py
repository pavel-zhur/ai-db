...
from console.models import CommandType, ConversationEntry, OutputFormat, SessionState
...
def test_command_type_enum() -> None:
...
def test_conversation_entry() -> None:
    ...
    # Test with error
    error_entry = ConversationEntry(timestamp=now, user_input="invalid query", error="Syntax error")
...
def test_session_state() -> None:
    ...
    state = SessionState(
        conversation_history=[], transaction_active=False, current_output_format=OutputFormat.TABLE
    )
    ...
    # Test adding entry
    state.add_entry(
        user_input="test query", response="test response", command_type=CommandType.QUERY
    )
    ...
    state.add_entry(user_input="bad query", error="Error message")
...
def test_session_state_with_transaction() -> None:
    """Test SessionState with transaction."""
    state = SessionState(conversation_history=[], transaction_active=True, transaction_id="tx-123")
