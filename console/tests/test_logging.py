...
def test_setup_logging(tmp_path):
    ...
    # Verify log file exists and contains info but not debug
    assert log_file.exists()
    log_contents = log_file.read_text()
    assert "Test message" in log_contents
    assert "Debug message" in log_contents  # File handler logs everything
...
def test_trace_logger_creates_directory(tmp_path):
