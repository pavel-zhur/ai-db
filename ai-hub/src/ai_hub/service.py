...
class AIHubService:
    ...
    @property
    def aidb(self) -> AIDB:
        """Get or create AI-DB instance."""
        if self._aidb is None:
            self._aidb = AIDB()
        return self._aidb
    ...
    def _convert_data_loss_indicator(self, indicator: Any) -> DataLossIndicator:
    ...
    async def _execute_with_progress(self, operation_coro, operation_desc: str) -> Any:
        ...
        try:
            ...
            progress_task = asyncio.create_task(
                self._progress_feedback(operation_desc)
            )
    ...
    async def execute_query(
        ...
        async def _execute():
            async with await git_layer.begin(
                self._config.git_repo_path,
                message=f"Execute query: {query[:50]}..."
            ) as transaction:
                try:
                    ...
                    return QueryResponse(
                        ...
                        data_loss_indicator=self._convert_data_loss_indicator(result.data_loss_indicator),
    ...
    async def execute_view(
        ...
        async def _execute():
            async with await git_layer.begin(
                self._config.git_repo_path,
                message=f"Execute view: {view_name}"
            ) as transaction:
                try:
    ...
    async def execute_data_modification(
        ...
        async def _execute():
            async with await git_layer.begin(
                self._config.git_repo_path,
                message=f"Data modification: {operation[:50]}..."
            ) as transaction:
                try:
